...
static hi_u32 I2C_WriteMutilByte(hi_u8* cmd, hi_u8 send_len)
{
    hi_u32 status;
    hi_i2c_idx id = 0;//I2C 0
    hi_u8 send_i2C_write_cmd_addr[1] = {0x00};//д
...
/*OLED write cmd*/
static hi_void write_cmd(hi_u8 cmd)//д����
{
    hi_u8 status = 0;
    /*д�豸��ַ*/
...
/*OlED write data*/
static hi_void write_data(hi_u8 I2C_Data)//д����
{
    hi_u8 status = 0;
    /*д�豸��ַ*/
...
{
    oled_fill_screen(0x00);//ȫ����
    hi_udelay(10000);//10ms
    oled_show_str(0,3, " Welcome 2026 ",1);
    hi_udelay(2000000);//20ms
}
...
hi_void *app_i2c_oled_demo(hi_void* param)
{
    ...
    while (1)
    {
        ...
        sprintf(temperature_str, "Temperature:%.0f", g_temperature);
        sprintf(humidity_str, "Humidity:%.0f", g_humidity);
        sprintf(pressure_str, "Pressure:%.0f", g_pressure);
        if (g_car_status == CAR_RUNNING_STATUS) {
            oled_show_str(0,2,"ultrasonic  mode",1); 
            oled_show_str(0,3,temperature_str,1);
            oled_show_str(0,4,humidity_str,1);
            oled_show_str(0,5,pressure_str,1);
         } else if (g_car_status == CAR_STOP_STATUS) {
            oled_show_str(0,2," Robot Car Stop ",1);
            oled_show_str(0,3,temperature_str,1);
            oled_show_str(0,4,humidity_str,1);
            oled_show_str(0,5,pressure_str,1);
        }  else if (g_car_status == CAR_TRACE_STATUS) {
            oled_show_str(0,2,"   trace mode   ",1);
            oled_show_str(0,3,temperature_str,1);
            oled_show_str(0,4,humidity_str,1);
            oled_show_str(0,5,pressure_str,1);
        }
